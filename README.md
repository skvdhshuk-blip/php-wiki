...
“保存为提案”只接受带当前有效结构化证据的正式回答。应用会把 `[^E*]` 转为持久 Wiki 的规范 source 引用并重新校验来源哈希；澄清、证据不足、未知或陈旧证据不会生成待审提案，重复保存会复用现有待审提案。
...
来源状态 `processed` 只表示扫描后的处理进度，是否获批只看 Proposal 状态和 Wiki Git commit。来源或页面在审批前发生变化会让提案进入 `stale`，不会覆盖用户改动。

审批成功后 FTS5 会尽力重建；索引失败不会回滚已经成功的 Wiki/Git/Proposal 权威提交，旧索引继续可用，并可执行 `php artisan php-wiki:rebuild-search` 恢复。
